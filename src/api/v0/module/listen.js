...
db.getListen = (idSong) => {
    return new Promise((resolve, reject) => {
        pool.query(`SELECT id_song, day , listenofday FROM listen WHERE id_song = $1 AND day >= (date_trunc('day', timezone('Asia/Ho_Chi_Minh'::text, now()))::date) - 10 ORDER BY day DESC`,
            [idSong],
            (err, result) => {
                if (err) return reject(err);
                return resolve(result.rows);
