...
db.getSong = (id_song, idAccount) => {
    return new Promise((resolve, reject) => {
        pool.query("SELECT  song.id_song, song.name_song, song.link, song.listen,song.image_song, count(love.id_song) as qtylove,song.description,song.song_status,account.id_account, account.account_name,song.id_album, album.name_album, song.created, exists(select 1 from love where love.id_account = $1) as lovestatus "
            + "FROM(((song "
